...
def alphabet_position(text: str) -> str:
    return ' '.join(map(str, [{letter: position + 1 for position, letter in enumerate(string.ascii_lowercase)}[i] for i in [i for i in text.lower() if i in string.ascii_lowercase]]))
