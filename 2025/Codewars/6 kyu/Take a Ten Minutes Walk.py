...
def is_valid_walk(walk: list) -> bool:
    coordinates = [0, 0]
    for i in walk:
        coordinates[0] += 1 if i == 'n' else -1 if i == 's' else 0
        coordinates[1] += 1 if i == 'w' else -1 if i == 'e' else 0
    return True if coordinates[0] == 0 and coordinates[1] == 0 and len(walk) == 10 else False
...
assert is_valid_walk(['e', 's', 'w', 'w', 'w', 'n', 'e', 'e']) == False
